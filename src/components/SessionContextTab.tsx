      ...
      <div className="alert bg-info/10 border border-info/20">
        ...
        <div>
          ...
          <div className="text-sm opacity-70">
            ...
            <br />
            We attempt to show when a file has been read in a session, this is currently not fully reliable (but we are working on it!).
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="badge badge-lg gap-2">
                ...
                {contextFiles.length}{" "}
                {contextFiles.length === 1 ? "file" : "files"} found
              </div>
...
function ContextFileCard({ file, expanded, onToggle, usageStats }: ContextFileCardProps) {
  ...
  const [syntaxHighlighterDeps, setSyntaxHighlighterDeps] = useState<SyntaxHighlighterDeps | null>(null);
  ...
  useEffect(() => {
    if (expanded && !depsChecked) {
      loadSyntaxHighlighterDeps().then(deps => {
        if (deps) {
  ...
  return (
    <div className={`card border border-base-300 ${isUsed ? 'bg-secondary/10 border-secondary/30' : 'bg-base-100'}`}>
      {/* File header */}
      <div
