...
const DEBOUNCE_DURATION: Duration = Duration::from_secs(1); // 1 second
...
impl ClaudeWatcher {
    ...
    fn file_event_processor(
        ...
        loop {
            ...
            match rx.recv_timeout(Duration::from_secs(5)) {
                Ok(Ok(event)) => {
                    if let Some(file_event) = Self::process_file_event(&event, &projects_path) {
                        if let Err(e) = log_info(
                            PROVIDER_ID,
                            &format!("📝 File modified: {} → Processing for upload", file_event.path.file_name().unwrap_or_default().to_string_lossy()),
                        ) {
                            eprintln!("Logging error: {}", e);
                        }
            ...
            // Check for files ready to upload (debounced)
            let now = Instant::now();
            let mut ready_files = Vec::new();

            for (path, file_event) in &pending_files {
                if now.duration_since(file_event.last_modified) >= DEBOUNCE_DURATION {
                ...
                if let Some(file_event) = pending_files.remove(&path) {
                    if let Err(e) = upload_queue.add_item(
                        ...
                        if let Err(log_err) = log_error(
                            PROVIDER_ID,
                            &format!("✗ Failed to queue file for upload: {}", e),
                        ) {
                            eprintln!("Logging error: {}", log_err);
                        }
                    }
                    // Upload queue will handle success/failure logging
                }
            }
        }
